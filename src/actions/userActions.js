...
export function registerUser(_id, _pass) {
  return function (dispatch) {
    ...
    .then((json) => {
      if (json.message == 'ok') { dispatch(loginLocal(_id, _pass)); } else { dispatch({ type: REGISTER_FAILURE, message: json.message }); }
    });
  };
}
export function loginLocal(_id, _pass, keepLogin = false) {
  return function (dispatch) {
    ...
    .then((json) => {
      if (json.token === undefined) { dispatch(failLogin(json)); } else {
...
export function successLogin(id, token, keepLogin = false, isTemp = false) {
  return (dispatch, getState) => {
    ...
    if (isTemp) { return dispatch({ type: LOGIN_TEMP, id }); }
